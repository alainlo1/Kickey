            ...
            <h4>Lorem ipsum dolor sit amet consectetur adipisicing elit. Qua voluptate libero sequi voluptas! Rerum?
                ...
                <div class="d_form" >
                            <form method="POST" id="le-form" action="page1?id=<?php echo $id ?>">
                                <input type="text" name="post_contenu" id='<?php echo $id ?> ' placeholder="Alors?">
                                <button type="submit" class="boutton" name="submit" value="Envoyer l'article">
                 ...
                 <p><?php if(isset($message)) { echo $message; } ?></p>
                    ...
                    // c'est pour afficher la tl une fois quon a poté
                        $articles = $bdd->prepare('SELECT posts.id 
                        AS post_id,post_contenu,cur_date,pseudo,id_utilisateurs,avatar 
                        FROM posts,utilisateurs WHERE id_jeux = ?
                         AND id_utilisateurs = utilisateurs.id  ORDER BY cur_date DESC');
